 ...
 * edges. We have discussed Dijkstra’s algorithm for this problem. Dijksra’s
 * algorithm is a Greedy algorithm and time complexity is O(VLogV) (with the use
 * of Fibonacci heap). Dijkstra doesn’t work for Graphs with negative weight
 ...
 * edge u-v ………………If dist[v] > dist[u] + weight of edge uv, then update dist[v]
 * ………………….dist[v] = dist[u] + weight of edge uv
 * 
 * 3) This step reports if there is a negative weight cycle in graph. Do
 * following for each edge u-v ……If dist[v] > dist[u] + weight of edge uv, then
 * “Graph contains negative weight cycle” The idea of step 3 is, step 2
 ...
 * that is why the outer loop runs |v| – 1 times. The idea is, assuming that
